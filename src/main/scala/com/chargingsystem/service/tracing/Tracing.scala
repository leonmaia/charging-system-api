package com.chargingsystem.service.tracing

import com.chargingsystem.server.ApiConfig
  ...
  def withTrace[T](id: String, protocol: String = "custom", timeout: Duration = fromSeconds(3)) (block: => Future[T]) = {
    if (ApiConfig.isDev) Trace.traceService(id, protocol, Option.empty) {
      ...
      withTimeout(id, timeout, block) map { res =>
        log.info(s"$id - Charging-System API took ${System.currentTimeMillis() - time}ms to respond")
